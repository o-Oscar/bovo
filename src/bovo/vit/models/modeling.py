...
import bovo.vit.models.configs as configs
import numpy as np
import torch
import torch.nn as nn
from bovo.wrap.network import ResNetBlock
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        # TODO : add resnet here : make sure ResNetV2 takes a single channel as input
        # but:
        if True:
            ...
            self.patch_embeddings = torch.nn.Sequential(
                ResNetBlock(in_channels, 8, False),
                ResNetBlock(8, 16, False),
                ResNetBlock(16, 32, False),
                Conv2d(
                    in_channels=32,
                    out_channels=config.hidden_size,
                    kernel_size=patch_size,
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        if labels is not None:
            logits = torch.reshape(logits, labels.shape)
...
class ResNet(nn.Module):
    def __init__(self, img_size=256, zero_head=False, vis=False):
        ...
        assert self.img_size % (2**n_modules) == 0
        self.coarse_img_size = self.img_size // (2**n_modules)

        # self.layers = [ResNetBlock(1, n_channels, False)]
        self.layers = [nn.Conv2d(1, n_channels, kernel_size=7, stride=2, padding=1)]
        ...
        self.mlp_size = n_channels * self.coarse_img_size * self.coarse_img_size
        print("mlp_size : ", self.mlp_size)
        print("coarse_img_size : ", self.coarse_img_size)

        self.head = Linear(self.mlp_size, 1)
    ...
    def forward(self, x, labels=None):
        x = self.layers(x)
        logits = self.head(torch.reshape(x, (x.shape[0], self.mlp_size)))
...
def set_parameter_requires_grad(model, require_grad: bool):
    for param in model.parameters():
        param.requires_grad = require_grad


class PretrainedResNet(nn.Module):
    def __init__(self, img_size=224, reg_loss=False, model_type="res18"):
        ...
        if model_type == "res18":
            ...
            raise NameError("model type unknown")

        set_parameter_requires_grad(self.backbone, False)
        num_ftrs = self.backbone.fc.in_features
        self.backbone.fc = nn.Linear(num_ftrs, 1)
        self.backbone.conv1 = Conv2d(
    ...
    def forward(self, x, labels=None):
        ...
        if labels is not None:
            ...
            if self.reg_loss:
                ...
                loss = self.bce(
                    self.sigmoid(logits).float(), (labels.float() > 17).float()
                )
            # exit()
            # loss_fct = CrossEntropyLoss()
            # loss = loss_fct(logits.view(-1, self.num_classes), labels.view(-1))
            y = np.asarray(
                labels.detach().cpu().numpy().flatten() > 17, dtype=np.float32
            )
            if self.reg_loss:
                y_ = np.asarray(
                    logits.detach().cpu().numpy().flatten() > 17, dtype=np.float32
                )
            else:
                y_ = np.asarray(
                    logits.detach().cpu().numpy().flatten() > 0, dtype=np.float32
                )
            accuracy = 100 * np.mean(y * y_ + (1 - y) * (1 - y_))
            # accuracy = (y, logits.detach().cpu().numpy().flatten())
